...
## Cross-device manual tests (2026-06-09)
...
| **A** | Studio page sync (phone ↔ Dell) | **PASSED** | v14 workspace-first restore |

| **B** | Practice field sync (section focus, groove, minutes) | **PASSED** | `97fad4a` session_section_wins |

| **C** | Backing content sync (BPM, scope, loops, groove, meter) | **PASSED** | v18 user-intent dirty gate; phone hard refresh restores Dell values |

| **D** | Active song + display key + instrument + page restore | **NEXT** | Do not reopen A/B/C unless new trace shows failure |
...
- Phone hard refresh restored Dell Backing values; no false `backing_dirty` / `local_edit_preserve` overwrite

- Trace: `backing_user_edit_intent=False`, `backing_restore_source=cloud_restore` after refresh without widget interaction
