...
def render_backing_composition_song_context_card(
    ...
    *,
    ...
    try:
        ...
        concert = html.escape(format_key_label(concert_raw))
    except ImportError:
        try:
            from musical_context_authority import format_practice_concert_key_line

            concert = html.escape(
                format_practice_concert_key_line(session, fallback=concert_raw)
            )
        except ImportError:
            concert = html.escape(concert_raw)
    ...
    try:
        ...
        source_meta = (
            ...
            '<span class="ui-studio-meta-badge-ico" aria-hidden="true">📀</span>'
            '<span class="ui-studio-meta-badge-label">Source</span>'
            '<span class="ui-studio-meta-badge-value">Composition</span>'
            "</span>"
        )
        style_meta = (
            '<span class="ui-studio-meta-badge tone-style">'
...
def render_backing_edit_source_action(
    ...
    *,
    ...
    label = return_to_source_button_label(ctx, session=session)
