...
def open_sbi(page: Page) -> bool:
    ...
    click_radio(page, "Song-Based Improvisation") or click_radio(page, "Play Song-Based") or click_radio(
        page, "Song-Based"
    ) or click_button_has(page, r"Song-Based Improvisation")
    settle(page, 3)
    ...
    if "song source" not in body:
        ...
        settle(page, 3)
        body = body_text(page).lower()
    return "song source" in body or "active song" in body and "custom progression" in body
...
def capture(page: Page, gate: str, step: str) -> dict[str, object]:
    ...
    for pat in (r"Practice Focus\s*·[^\n]{0,200}", r"🔍\s*Practice Focus[^\n]{0,200}"):
        ...
        if fm:
            ...
            break
    sidebar_song = ""
    if re.search(r"Trial Song", active_block, re.I):
    ...
    row = {
        ...
        "practice_key": pk,
        "focus": focus,
        "tab_sbi": bool(re.search(r"Song source", main, re.I)),
