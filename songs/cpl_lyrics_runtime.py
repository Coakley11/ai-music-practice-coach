...
CPL_LYRICS_EXPANDER_KEY = "_cpl_lyrics_expander_open"
...
def _persist_cpl_lyrics_library_copy(session_state: dict[str, Any], active: dict[str, Any]) -> None:
    """Mirror lyrics into the saved custom song library entry when one exists."""
    from custom_progression_lab import CPL_SAVED_KEY, save_progression

    song_name = str(active.get("name") or "").strip()
    if not song_name:
        return
    store = dict(session_state.get(CPL_SAVED_KEY) or {})
    if not isinstance(store, dict) or not store:
        return
    active_id = str(active.get("id") or "").strip()
    target_name = ""
    if song_name in store:
        target_name = song_name
    elif active_id:
        for name, blob in store.items():
            if isinstance(blob, dict) and str(blob.get("id") or "").strip() == active_id:
                target_name = str(name)
                break
    if not target_name:
        return
    save_progression(store, target_name, active)
    session_state[CPL_SAVED_KEY] = store


def save_cpl_lyrics_to_active(
    session_state: dict[str, Any],
    active: dict[str, Any],
    *,
    cpl_active_key: str = "cpl_active_progression",
) -> dict[str, Any]:
    """Persist section lyrics into active CPL blob + saved library + disk/cloud."""
    from custom_progression_lab import ensure_original_structure

    lyrics = collect_cpl_lyrics_from_session(session_state, active)
    active = ensure_original_structure(dict(active))
    active[CPL_LYRICS_BY_SECTION_KEY] = lyrics
    session_state[cpl_active_key] = active
    _persist_cpl_lyrics_library_copy(session_state, active)
    ...
    try:
        ...
        session_state[CPL_LYRICS_NOTICE_KEY] = (
            f"Saved lyrics locally; sync will retry on next save ({exc})."
        )
    return active
...
def render_cpl_lyrics_editor_panel(
    ...
    *,
    ...
    """Compact optional lyrics editor for the active Custom Progression on Songs page."""
    from custom_progression_lab import ensure_original_structure
    ...
    stored = dict(active.get(CPL_LYRICS_BY_SECTION_KEY) or {})
    expander_label = "Lyrics"
    if stored:
        expander_label = f"Lyrics ({len(stored)} section{'s' if len(stored) != 1 else ''} saved)"

    with st.expander(expander_label, expanded=bool(st.session_state.get(CPL_LYRICS_EXPANDER_KEY))):
        st.caption(
            "Add lyrics by section for your custom song. Saved with this song on this device "
        ...
        with col_save:
            if st.button("Save lyrics", key="cpl_save_lyrics_btn", use_container_width=True):
                st.session_state[CPL_LYRICS_EXPANDER_KEY] = True
                updated = save_cpl_lyrics_to_active(
                    st.session_state,
                    active,
