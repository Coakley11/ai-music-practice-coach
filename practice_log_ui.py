...
def render_practice_log_diagnostics(
    ...
    *,
    ...
    with st.expander("Practice Log persistence diagnostics (?dev=1)", expanded=False):
        ...
        st.text(f"latest generated_at: {cc.get('generated_at')}")
        st.text(f"report_date: {cc.get('report_date')}")
        st.text(f"resume upsert result: {cc.get('resume_upsert_result')}")
        st.text(f"music resume upsert result: {cc.get('music_resume_upsert_result')}")
        st.text(f"activity record/update result: {cc.get('activity_record_result')}")
        st.text(f"activity skipped reason: {cc.get('activity_skipped_reason')}")
...
__all__ = [
    ...
    "render_practice_log_page",
    "submit_analyze_practice_to_ami",
]
