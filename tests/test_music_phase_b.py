...
class TestMusicWorkspaceEnvelope(unittest.TestCase):
    ...
    def test_resolve_page_change_prefers_hint_over_stale_studio_page(self) -> None:
        ...
        self.assertEqual(source, "normalized_studio_page")
...
class TestPersistenceTracePanel(unittest.TestCase):
    def test_deploy_probe_uses_persist_deploy_version(self) -> None:
        ...
        self.assertIn("page-change-save-stamp-v21-hotfix-transposing-kwarg", info["build_marker"])
    ...
    def test_finalize_uses_normalized_studio_page_over_stale_picker_hint(self) -> None:
        ...
        self.assertEqual(trace.get("save_payload_core_page"), "backing")
        self.assertEqual(trace.get("pre_stamp_core_page"), "picker")
        self.assertEqual(trace.get("final_payload_source"), "normalized_studio_page")

    def test_finalize_page_change_stamps_picker_core_to_backing_before_write(self) -> None:
        from music_persistent_state import finalize_music_page_change_cloud_payload
        ...
        out, trace = finalize_music_page_change_cloud_payload(st, state)
        self.assertEqual(out["core"]["studio_page"], "backing")
        self.assertEqual(out["music_workspace_state"]["studio_page"], "backing")
        self.assertEqual(trace.get("pre_stamp_core_page"), "picker")
        self.assertEqual(trace.get("post_stamp_core_page"), "backing")
        self.assertEqual(trace.get("final_payload_studio_page"), "backing")
        self.assertEqual(trace.get("final_payload_source"), "normalized_studio_page")
    ...
    def test_resolve_music_save_reason_pending_stamp_wins_over_song_edit(self) -> None:
        from music_persistent_state import resolve_music_save_reason_at_write
    ...
    def test_build_music_state_for_save_uses_fallback_when_build_throws(self) -> None:
        from music_persistent_state import build_music_state_for_save
        ...
        def _boom(_st: Any) -> dict[str, Any]:
            raise RuntimeError("commit failed")

        state = build_music_state_for_save(st, _boom, save_reason="page_change")
        self.assertEqual(state["core"]["studio_page"], "backing")
        self.assertTrue(st.session_state.get("_music_fallback_page_change_build"))

    def test_save_music_cloud_session_records_write_path_and_payload(self) -> None:
        from music_persistent_state import save_music_cloud_session, stamp_music_payload_for_write
        ...
        state = stamp_music_payload_for_write(
            st, state, explicit_reason="page_change", write_path="force_autosave"
        )
        with patch("suite_cloud_state.save_cloud_full_session", return_value=True):
            ok = save_music_cloud_session(
                st,
                state,
                write_path="force_autosave",
                page="backing",
        ...
        self.assertTrue(ok)
        self.assertEqual(st.session_state.get("_music_cloud_write_path"), "force_autosave")
        self.assertEqual(st.session_state.get("_music_cloud_payload_studio_page"), "backing")
        self.assertEqual(st.session_state.get("_music_cloud_payload_source"), "last_write")
    ...
    def test_stamp_music_payload_song_edit_with_pending_page_change_finalizes_backing(self) -> None:
        ...
        out = stamp_music_payload_for_write(
            st,
            state,
            explicit_reason="song_edit",
            write_path="force_autosave",
        )
        self.assertEqual(out["core"]["studio_page"], "backing")
        self.assertEqual(st.session_state.get("_music_save_reason_at_write"), "page_change")
        self.assertTrue(st.session_state.get("page_change_finalize_ran"))
        self.assertEqual(st.session_state.get("_music_cloud_write_studio_page"), "backing")
