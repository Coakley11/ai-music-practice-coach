...
def render_custom_progression_lab_page() -> None:
    ...
    app._render_page_quick_nav("custom")
    app.compact_page_title(
        "✏️",
        "Custom Progression",
    ...
    if st.button("Use as active song", key="cpl_set_active_source", type="primary"):
        ...
        note_active_source_change(st, invalidate_backing=app.invalidate_backing_cache)
        st.rerun()

    if app.is_custom_progression(st.session_state):
    ...
    if CPL_ACTIVE_KEY not in st.session_state:
        st.session_state[CPL_ACTIVE_KEY] = app.default_active_progression()
    ...
    cpl_practice_key = app.display_key
    ...
    # --- Step 1: Style ---
    st.markdown('<div class="cpl-step-card"><span class="cpl-step-num">1</span><span class="cpl-step-title">Choose style</span></div>', unsafe_allow_html=True)
    ...
    # --- Step 2: Key ---
    st.markdown('<div class="cpl-step-card"><span class="cpl-step-num">2</span><span class="cpl-step-title">Choose key</span></div>', unsafe_allow_html=True)
    key_buttons = ["C", "F", "Bb", "Eb", "G", "D", "Am", "Em"]
    kcols = st.columns(len(key_buttons))
    for i, k in enumerate(key_buttons):
        with kcols[i]:
            if st.button(k, key=f"cpl_quick_key_{k}", use_container_width=True):
    ...
    st.markdown('<div class="cpl-step-card"><span class="cpl-step-num">3</span><span class="cpl-step-title">Build progression</span></div>', unsafe_allow_html=True)
    ...
    with tool_a:
        if st.button("➕ Add chord", key="cpl_add_chord", use_container_width=True):
            ...
            st.rerun()
    with tool_b:
        bulk = st.text_input("Paste chords", placeholder="Dm7 | G7 | Cmaj7", key="cpl_bulk_line", label_visibility="collapsed")
    with tool_c:
    ...
    # --- Step 4: Playback ---
    st.markdown('<div class="cpl-step-card"><span class="cpl-step-num">4</span><span class="cpl-step-title">Practice & playback</span></div>', unsafe_allow_html=True)
    ...
    cpl_groove = app.infer_groove_style({}, active.get("groove_style", "Auto"))
    cpl_sig = backing_signature(display_key, display_sections, active.get("bpm", 100), active.get("loops", 2), cpl_groove)

    play_c1, play_c2, play_c3 = st.columns(3)
    with play_c1:
        gen = st.button("▶ Generate backing", key="cpl_gen_backing", disabled=not cpl_events, use_container_width=True)
    with play_c2:
    ...
    if gen and cpl_events:
        st.session_state["cpl_backing_wav"] = app.generate_backing_track(
            cpl_events,
    ...
    coach_ctx = lab_context_for_coaching(display_sections, cpl_practice_key, app.instrument, app.level, app.focus)
    if coach_ctx["first_chords"]:
        st.info(
            app._section_overlay(
                app.instrument,
                app.focus,
